...
import { TaskerState, TaskerLocation, TaskerStats, Order } from '@/types';
import { mockTaskerService } from '@/src/api/mockServices';
import { createPersistentStore } from '@/src/store/utils/persistentStore';

interface TaskerStore extends TaskerState {
  ...
  fetchEarnings: (taskerId: string, period?: 'today' | 'week' | 'month') => Promise<void>;
    ...
    (set, get) => ({
      ...
      fetchAvailableJobs: async (taskerId: string, latitude: number, longitude: number) => {
        try {
          ...
          const response = await mockDriverService.getAvailableJobs(taskerId, latitude, longitude);
          if (response.success) {
      ...
      goOnlineAsync: async (taskerId: string) => {
        try {
          set({ isLoading: true, error: null });
          const response = await mockDriverService.goOnline(taskerId);
          if (response.success) {
      ...
      goOfflineAsync: async (taskerId: string) => {
        try {
          set({ isLoading: true, error: null });
          const response = await mockDriverService.goOffline(taskerId);
          if (response.success) {
      ...
      fetchStats: async (taskerId: string) => {
        try {
          set({ isLoading: true, error: null });
          const response = await mockDriverService.getStats(taskerId);
          if (response.success) {
      ...
      fetchEarnings: async (taskerId: string, period: 'today' | 'week' | 'month' = 'today') => {
        try {
          set({ isLoading: true, error: null });
          const response = await mockDriverService.getEarnings(taskerId, period);
          if (response.success) {
            set(state => ({
