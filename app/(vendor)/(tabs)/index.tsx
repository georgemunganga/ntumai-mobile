// Dashboard Tab - Vendor Dashboard
// Migrated from screens/vendor/VendorDashboard.tsx
    ...
    <View className='flex-1'>
      ...
      <View className="bg-white px-4 py-4 border-b border-gray-200">
        <View className="flex-row items-center justify-between">
          <View>
            <AppText style={{fontFamily:'Ubuntu-Bold'}} className="text-2xl font-bold text-gray-900">Dashboard</AppText>
            <AppText style={{fontFamily:'Ubuntu-Bold'}} className="text-gray-500 mt-1">Welcome back, Vendor!</AppText>
          </View>
      ...
      <ScrollView className="flex-1">
            ...
            {stats.map((stat, index) => (
              <View key={index} className="w-1/2 px-2 mb-4">
                <View className="bg-white p-4 rounded-xl">
                  ...
                  <AppText style={{fontFamily:'Ubuntu-Bold'}} className="text-2xl font-bold text-gray-900 mb-1">
                    {stat.value}
                  </AppText>
                  <AppText className="text-gray-500 text-sm">{stat.title}</AppText>
                </View>
              </View>
            ))}
          </View>
        ...
        <View className="px-4 mb-6">
          <AppText style={{fontFamily:'Ubuntu-Bold'}} className="text-lg font-semibold text-gray-900 mb-4">Quick Actions</AppText>
          <View className="flex-row justify-between">
            {[
              ...
              <TouchableOpacity key={index} className="items-center">
                ...
                <AppText style={{fontFamily:'Ubuntu-Medium'}} className="text-xs text-gray-600 text-center">{action.title}</AppText>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View className="px-4 mb-6">
          <View className="flex-row items-center justify-between mb-4">
            <AppText style={{fontFamily:'Ubuntu-Bold'}} className="text-lg font-semibold text-gray-900">Recent Orders</AppText>
            <TouchableOpacity>
              <AppText style={{fontFamily:'Ubuntu-Medium'}} className="text-emerald-600 font-medium">View All</AppText>
            </TouchableOpacity>
          </View>
                ...
                <View className="flex-row items-center justify-between">
                  <View className="flex-1">
                    <View className="flex-row items-center mb-1">
                      <AppText style={{fontFamily:'Ubuntu-Medium'}} className="font-semibold text-gray-900 mr-2">{order.id}</AppText>
                      <View className={`px-2 py-1 rounded-full ${
                        order.status === 'Preparing' ? 'bg-yellow-100' :
                        order.status === 'Ready' ? 'bg-green-100' : 'bg-gray-100'
                      }`}>
                        <AppText style={{fontFamily:'Ubuntu-Medium'}} className={`text-xs font-medium ${
                    ...
                    <AppText style={{fontFamily:'Ubuntu-Medium'}} className="text-gray-600 text-sm">{order.customer}</AppText>
                    <AppText style={{fontFamily:'Ubuntu-Medium'}} className="text-gray-400 text-xs mt-1">{order.time}</AppText>
                  </View>
                  <AppText style={{fontFamily:'Ubuntu-Medium'}} className="font-bold text-gray-900">{order.amount}</AppText>
                </View>
              </View>
            ))}
          </View>
